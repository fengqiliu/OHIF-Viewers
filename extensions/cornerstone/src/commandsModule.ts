...
function commandsModule({
  ...
  const actions = {
    ...
    // ============================================================
    // CPR (Curved Planar Reformation) commands
    // ============================================================

    generateCPRReformation: async ({ pathId, config }) => {
      const { cprService, uiNotificationService } = servicesManager.services;

      try {
        const result = await cprService.createReformation(pathId, config);
        uiNotificationService.show({
          title: 'CPR Generated',
          message: `Reformation generated: ${result.width}x${result.height} pixels`,
          type: 'success',
          duration: 3000,
        });
        return result;
        ...
        uiNotificationService.show({
          title: 'CPR Error',
          message: `Failed to generate reformation: ${error.message}`,
          type: 'error',
          duration: 5000,
        });
        throw error;
    ...
    displayCPRReformation: ({ imageId }) => {
      ...
      if (viewport && viewport instanceof StackViewport) {
        ...
        // Need to get a stack viewport - try to use the active one
        const enabledElement = getEnabledElement(
          cornerstoneViewportService.getViewportElement(activeViewportId)
        );
        if (enabledElement?.viewport instanceof StackViewport) {
          enabledElement.viewport.setStack([imageId]);
  ...
  const definitions = {
    ...
    // CPR commands
    generateCPRReformation: {
      commandFn: actions.generateCPRReformation,
    },
