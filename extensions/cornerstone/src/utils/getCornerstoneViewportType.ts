...
export default function getCornerstoneViewportType(
  ...
  if (lowerViewportType === CPR) {
    // CPR uses stack viewport for displaying reformed slices
    return Enums.ViewportType.STACK;
  }

  throw new Error(
    `Invalid viewport type: ${viewportType}. Valid types are: stack, volume, video, wholeslide, cpr`
  );
}
