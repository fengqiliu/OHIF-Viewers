...
import { onCompletedCPRPath } from './tools/CPRPathTool';
...
const initMeasurementService = (
  ...
  measurementService.addMapping(
    csTools3DVer1MeasurementSource,
    'CPRPath',
    CPRPath.matchingCriteria,
    CPRPath.toAnnotation,
    CPRPath.toMeasurement
  );
...
const connectToolsToMeasurementService = ({
  ...
  function addMeasurement(csToolsEvent) {
    try {
      ...
      } else if (csToolsEvent.type === completedEvt && toolName === toolNames.CPRPath) {
        // Register CPR path with CPRService, then add as measurement
        annotationAddedEventDetail.uid = annotationUID;
        annotationToMeasurement(toolName, annotationAddedEventDetail);
        onCompletedCPRPath(servicesManager, csToolsEvent);
      } else {
